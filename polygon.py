...
class Polygon:
    """Polygon is a class of geometric extraction. In this implementation, the polygon is a parallelogram set by its center, its size, its rotation and its transvection."""
    def __init__(self,center,size,angleRotation=0,angleInclinaison=0,outSize=(32,32)):
        """Create a Polygon."""
        self.center = center
        self.size = size
        self.angleRotation = angleRotation
        self.angleInclinaison = angleInclinaison
    ...
    def corners(self):
        ...
        polygonCorners = ((int(xy2[0,0]),int(xy2[1,0])),(int(xy2[0,1]),int(xy2[1,1])),(int(xy2[0,2]),int(xy2[1,2])),(int(xy2[0,3]),int(xy2[1,3])))
    ...
    def mapMatrix(self):
        ...
        mapMatrix[0,0] = sx*numpy.cos(self.angleRotation)
        mapMatrix[0,1] = sy*(numpy.cos(self.angleRotation)*numpy.sin(self.angleInclinaison)-numpy.sin(self.angleRotation))
        mapMatrix[1,0] = sx*numpy.sin(self.angleRotation)
        mapMatrix[1,1] = sy*(numpy.sin(self.angleRotation)*numpy.sin(self.angleInclinaison)+numpy.cos(self.angleRotation))
